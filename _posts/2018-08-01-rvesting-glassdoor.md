...
In the R code shown previously, note that I used the `map_dfr` function from the `purr` package to bind together the rows from each results page (from 1 to the maximum) in order to create a single tidy dataframe.
